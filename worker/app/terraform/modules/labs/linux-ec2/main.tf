...
# Public subnet — EC2 gets public IP for outbound internet access
# Inbound blocked by security group, so no inbound internet access
...
# Route table — outbound to Internet Gateway
...
resource "aws_instance" "node" {
  ...
  user_data = base64encode(templatefile("${path.module}/userdata.sh", {
    ...
    node_role       = var.nodes[count.index].role
    ssh_public_key  = tls_private_key.lab_ssh.public_key_openssh
  }))

  tags = {
    Name          = "${var.environment_name}-${var.nodes[count.index].name}"
    node-role     = var.nodes[count.index].role
    lab-session   = var.session_id
    managed-by    = "devops-lab-platform"
  }
}
